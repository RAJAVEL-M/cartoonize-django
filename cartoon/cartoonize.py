...
import cartoon.network as network
import cartoon.guided_filter as guided_filter
#import network
...
def cartoon_fun(img):
    ...
    wbc = WB_Cartoonize('cartoon/saved_models/', gpu)
    p='c://django/cartoonize_django'
    img=p+img
    print("img",img)
    outputfile=img[:-4]+'output.jpg'
    img = cv2.imread(img)
    #print("imread",img)
    ...
    #cv2.imshow("output",cartoon_image)
    cv2.imwrite(outputfile, cartoon_image.astype(np.uint8))
    #plt.show()
    return outputfile

#cartoon_fun('baby1.jpg')
